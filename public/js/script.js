...
(() => {
  ...
  let latestAudioStatus = "success"; // Latest status (sound shader)
  ...
  window.addEventListener(
    ...
    () => {
      ...
      download.addEventListener(
        "click",
        () => {
          // Exit immediately if the button is disabled or encoding is in progress
          if (
          ...
          // Display a dialog to set parameters for download
          ...
          // Enable/Disable when there is a change in the radio button
          const radioListener = () => {
            const flag =
          ...
          showDialog(wrap, { okLabel: "start" }).then((isOk) => {
            ...
            // Add 'disabled' to prevent continuous pressing
            download.classList.add("disabled");
            // Change the label of the download button
            download.textContent = "generate...";
            // Set the encoding flag
            isEncoding = true;
            ...
            // Capture start by getting various parameters from the DOM
            setTimeout(() => {
              switch (formatName) {
                case "gif":
                case "webm":
        ...
        async () => {
          ...
          if (snapshotLink != null) {
            if (currentChannelId == null) {
              ...
              // Change the state of the omnibar (address bar) to the same URL as the share link
              // If broadcasting, prioritize that
              history.replaceState("", "", snapshotLink);

              // Hide viewer count and star count
              // Do not hide if broadcasting
              hideViewerIcon();
              hideStarIcon();
            }
